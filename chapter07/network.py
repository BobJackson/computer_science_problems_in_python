...
from chapter07.layer import Layer
from chapter07.util import sigmoid, derivative_sigmoid
...
class Network:
    ...
    def train(self, inputs: List[List[float]], expects: List[List[float]]) -> None:
        for location, xs in enumerate(inputs):
            ys: List[float] = expects[location]
            outs: List[float] = self.outputs(xs)
            self.backpropagate(ys)
            self.update_weights()

    def validate(self, inputs: List[List[float]], expects: List[T], interpret_output: Callable[[List[float]], T]) -> \
            Tuple[int, int, float]:
        correct: int = 0
        for input, expected in zip(inputs, expects):
